...
# these fields give more detailed song features
# using these fields, i have noticed the model accuracy level is lower
...
# get all data from the playlist
...
def get_song_data(song_dict):
    # get song ids only from playlist
    song_list = []
...
# each function can be applied to my good, bad, and discover playlists
...
# i need my song ids in an array to pass through the spotify function
...
def get_sp_audio(song_ids, song_id_list):
    # get audio analysis data for all songs in playlist
...
def get_audio_analysis(song_id_list):
    # get specific fields from data i'll use in the model
    app = []
...
def concat_song_ids(data, song_ids):
    # because the audio analysis call doesn't return a song idea, i have to concatenate
    # with my song ideas i used in the original spotify call
...
# label my good and bad songs with a target for modeling
gs_df['target'] = 1
bs_df['target'] = 0
...
def optimal_model_level():
    # in order to select the optimal k level and n_estimators level, i need to graph
    # the levels and their accuracy
    ...
    # plot the accuracy score by the level of k and n
    plt.subplot(1, 2, 1)
...
def get_knn_ds_results(results, df):
    # the output from my model comes in a list of 0's and 1's
    # each value corresponds to a song
    # because of this, i have to take a vertical stack of the list and add it in to the dataframe
    # i only want values with a '1' (songs it is predicted i will like)
...
def get_rf_ds_results(results, df):
    # see knn model above for explanation
...
def get_final_ds_songs(model_binary_name):
    ...
    # i will need to enter the column that corresponds to the model with the highest accuracy score
    # another method is including songs where both models predict i will like the song
...
n = len(ds_list)
while n > 0:
    # only one song can be uploaded at a time to spotify
    # when i loop through this, i'm creating a list of songs to upload
    # then , i clear the list and begin the loop again
    n -= 1
    k = n+1
